...
// ============================================================================
// RETRO BLUE SCOOTER ILLUSTRATION (Exact match of the reference cartoon vector)
// ============================================================================
export function RetroBlueScooter({ className = "w-full h-full" }: { className?: string }) {
  return (
    <svg viewBox="0 0 400 320" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
      <defs>
        {/* Gradients for rich 3D cartoon shading */}
      ...
      {/* 5. Exhaust Muffler Pipe (Dark Gunmetal) */}
      <g id="exhaust">
      ...
      {/* 6. Rear Engine Bodywork & Fender (Blue with White Side Panel) */}
      <g id="rear-chassis">
        {/* Main Blue Rear Fender Hood */}
        ...
        {/* Side Vents Air Slits on White Cowl */}
      ...
      {/* 7. Footboard Platform & Chrome Side Trim */}
      <g id="footboard">
      ...
      {/* 10. Classic Contoured Double Seat (Saddle) */}
      <g id="seat">
        {/* Base Rim */}
        <path d="M 90 152 C 85 140 105 120 140 120 C 180 120 195 138 200 150 Z" 
              fill="#0F172A" />
        {/* Cushioned Saddle in Blue Gradient */}
      ...
      {/* 12. Sparkle Gloss / Reflection Highlights */}
...
// ============================================================================
// SLEEK TECH REEL SCOOTER ILLUSTRATION (For Reel-to-Skill View)
// ============================================================================
export function TechReelScooter({ className = "w-full h-full" }: { className?: string }) {
  return (
    <svg viewBox="0 0 400 320" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
      ...
      {/* Ground Shadow with Cyber Blue Aura */}
      ...
      {/* Rear Aerodynamic Cowl & Battery Pack */}
      ...
      <path d="M 95 138 Q 145 125 195 138" stroke="#38BDF8" strokeWidth="2" />

      {/* Streamlined Cyber Headlight Unit */}
...
// Mini Blue Cartoon Vehicle 1 (For Card 1)
export function MiniBlueScooterA() {
  return (
    <div className="w-16 h-12 flex items-center justify-center">
      <RetroBlueScooter className="w-full h-full drop-shadow-sm scale-110" />
    </div>
  );
}

// Mini Blue Cartoon Vehicle 2 (For Card 2)
export function MiniBlueScooterB() {
  return (
    <div className="w-16 h-12 flex items-center justify-center">
      <TechReelScooter className="w-full h-full drop-shadow-sm scale-110" />
    </div>
  );
}
...
export function MyLearningScreen() {
  ...
  // Sub-detail navigation modal / active detail
  const [activeModal, setActiveModal] = useState<'none' | 'modules-list' | 'interview-tips' | 'stats'>('none');
  ...
  const onTouchEndHandler = () => {
    ...
    if (isLeftSwipe && activeCategory === 'career') {
      setActiveCategory('reel');
    }
    if (isRightSwipe && activeCategory === 'reel') {
      setActiveCategory('career');
    }
  };

  return (
    <div className="w-full min-h-screen bg-[#F0F4F8] flex justify-center selection:bg-sky-200 select-none pb-20 overflow-y-auto">
      
      {/* Mobile-First Frame Container matching screenshot */}
      <div 
        onTouchStart={onTouchStartHandler}
        onTouchMove={onTouchMoveHandler}
        onTouchEnd={onTouchEndHandler}
        className="w-full max-w-md min-h-screen bg-white shadow-2xl relative flex flex-col justify-between overflow-hidden"
      >
        
        {/* =========================================================================
            1. TOP STATUS & NAVIGATION BAR (Exact Replica of Mockup Header)
           ========================================================================= */}
        <div className="pt-3 px-6 shrink-0 space-y-2 z-20">
          
          {/* Mockup Status Bar: 10:41 | Wifi & Battery */}
          ...
          <div className="flex items-center justify-between pt-2">
            ...
            {/* Scroll / Swipe page indicator (Scroll 1/2 or Scroll 10) */}
            <div 
              onClick={() => setActiveCategory(activeCategory === 'career' ? 'reel' : 'career')}
              className="text-[11px] font-extrabold text-slate-600 px-2 py-1 rounded-lg hover:bg-slate-100 cursor-pointer transition-colors"
            >
            ...
            <button 
              ...
              className="w-9 h-9 rounded-full hover:bg-slate-100 flex items-center justify-center text-slate-800 transition-colors cursor-pointer"
            >
        ...
        {/* =========================================================================
           ...
           ========================================================================= */}
        <div className="relative w-full h-[320px] flex items-center justify-center px-4 overflow-hidden shrink-0">
          ...
          {/* Swipe Indicator Arrows (Subtle for convenience) */}
          <button 
            onClick={() => setActiveCategory('career')}
            className={`absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white/80 backdrop-blur-md shadow-sm border border-slate-200/60 flex items-center justify-center text-slate-600 z-10 transition-all ${activeCategory === 'career' ? 'opacity-30 cursor-default' : 'opacity-90 hover:scale-105 cursor-pointer'}`}
          >
          ...
          <button 
            ...
            className={`absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white/80 backdrop-blur-md shadow-sm border border-slate-200/60 flex items-center justify-center text-slate-600 z-10 transition-all ${activeCategory === 'reel' ? 'opacity-30 cursor-default' : 'opacity-90 hover:scale-105 cursor-pointer'}`}
          >
            <ArrowRight className="w-4 h-4 stroke-[2.5]" />
          </button>

          {/* DYNAMIC SWIPEABLE CARTOON VEHICLE DISPLAY */}
          <div className="relative z-10 w-full max-w-[320px] h-[260px] flex items-center justify-center transition-all duration-500 ease-out">
            {activeCategory === 'career' ? (
              <div 
                key="career-banner" 
                className="w-full h-full flex flex-col items-center justify-center animate-in fade-in zoom-in-95 duration-400"
              >
          ...
          {/* Swipe Dots Indicator at the bottom of the cartoon stage */}
        ...
        <div className="px-6 py-3 space-y-3 shrink-0 z-20">
          ...
          {/* TWO SIDE-BY-SIDE SOFT BLUE CARDS (Exact match of bottom layout) */}
          <div className="grid grid-cols-2 gap-3.5">
            ...
            >
              {/* Top Graphic + Price/Module Badge */}
        ...
        <div className="px-6 pt-2 pb-3 shrink-0 z-20">
          <div className="flex items-center gap-3">
            ...
            {/* Left Pill: User Avatar + Name ("Jesus Peters" / Learner Name) */}
            <div 
            ...
            {/* Right Solid Blue Button: "Aall now" / "Call now" with Chevron Up */}
            <button
              onClick={() => {
                if (activeCategory === 'career') {
        ...
        <div className="px-6 pb-6 pt-1 flex items-center justify-between shrink-0 z-20">
          ...
          {/* Icon 1: Twitter / Social Bird Icon */}
          <button 
            onClick={() => navigate('interview-prep')}
            className="w-11 h-10 rounded-2xl bg-[#E2EAF4] hover:bg-[#D3E0EE] active:scale-95 flex items-center justify-center text-slate-700 transition-all cursor-pointer"
            title="Interview Prep"
          >
            <MessageSquare className="w-4 h-4" />
          </button>

          {/* Icon 2: Search / Explore */}
          <button 
            onClick={() => navigate('choose-skill')}
            className="w-11 h-10 rounded-2xl bg-[#E2EAF4] hover:bg-[#D3E0EE] active:scale-95 flex items-center justify-center text-slate-700 transition-all cursor-pointer"
