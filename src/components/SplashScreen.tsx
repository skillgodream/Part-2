...
export function SplashScreen({ onComplete }: SplashScreenProps) {
  ...
  useEffect(() => {
    ...
    // 3. Begin smooth 800ms dissolve exit at 4.2 seconds
    const dissolveTimer = setTimeout(() => {
      setAnimationState('dissolving');
    }, 4200);

    // 4. Complete and unmount cleanly at exactly 5.0 seconds (5000ms)
      ...
      <div 
        ...
        style={{
          ...
          transform: isMounting 
            ? 'scale(0.90) translateY(6px)' 
            : isDissolving
            ? 'scale(1.02) translateY(-4px)'
            : 'scale(1) translateY(0px)',
          filter: isMounting ? 'blur(6px)' : isDissolving ? 'blur(3px)' : 'blur(0px)',
          transitionDuration: isDissolving ? '800ms' : '1800ms',
        }}
      >
        ...
        <div className="flex items-center justify-center">
          <h1
            className="text-4xl sm:text-6xl md:text-7xl lg:text-8xl font-bold text-black tracking-[0.14em] sm:tracking-[0.2em] md:tracking-[0.24em] uppercase select-none transition-all ease-out"
            style={{
